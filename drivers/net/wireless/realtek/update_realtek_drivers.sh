...
_update_realtek_driver() {
   ...
   then
      echo
      echo "Error: unknown chipset: $chipset"
      exit 1
   fi
   ...
   # remove old stuff, and copy over new stuff
   rm -rf $chipset/*
...
sed -i 's/CONFIG_PLATFORM_ARM_RPI = n/CONFIG_PLATFORM_ARM_RPI = n'/ rtl8192eu/Makefile
...
sed -i 's/CONFIG_RTL8821A = y/CONFIG_RTL8821A = n'/                     rtl8812au/Makefile
